      ...
      try {
        ...
        if (res.ok && data) {
          setForm({
            ...
            magazine_slug: data.slug || "",
            magazine_description: data.description || "",
            magazine_tags: data.tags || "",
            magazine_category: data.category || "",
            magazine_link: data.link || "",
            MagCloudLink: data.magCloudLink || "",
            magazine_date: data.date ? new Date(data.date).toISOString().split("T")[0] : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        toast.error(data.error || "Failed to update magazine issue");
      }
    } catch (err) {
      console.error(err);
      toast.error("Error updating magazine issue");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        toast.error(data.error || "Failed to delete magazine issue");
      }
    } catch (err) {
      console.error(err);
      toast.error("Error deleting magazine issue");
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between border-b pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">Edit Magazine Issue</h1>
            <p className="text-xs text-slate-500">Update issue details, cover image, links, and status.</p>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900/40 hover:bg-red-100 dark:hover:bg-red-950/40 rounded-lg px-3 py-1.5 text-xs font-bold transition cursor-pointer"
        >
